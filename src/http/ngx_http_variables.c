...
static ngx_int_t ngx_http_variable_is_args(ngx_http_request_t *r,
    ngx_http_variable_value_t *v, uintptr_t data);
static ngx_int_t ngx_http_variable_is_main_request(ngx_http_request_t *r,
    ngx_http_variable_value_t *v, uintptr_t data);
static ngx_int_t ngx_http_variable_alternate_uri(ngx_http_request_t *r,
    ngx_http_variable_value_t *v, uintptr_t data);
static ngx_int_t ngx_http_variable_is_proxy(ngx_http_request_t *r,
    ngx_http_variable_value_t *v, uintptr_t data);
static ngx_int_t ngx_http_variable_superresolution_time(ngx_http_request_t *r,
    ngx_http_variable_value_t *v, uintptr_t data);
static ngx_int_t ngx_http_variable_compute_queue_size_cpu(ngx_http_request_t *r,
    ngx_http_variable_value_t *v, uintptr_t data);
static ngx_int_t ngx_http_variable_compute_queue_size_gpu(ngx_http_request_t *r,
    ngx_http_variable_value_t *v, uintptr_t data);
static ngx_int_t ngx_http_variable_cache_hit(ngx_http_request_t *r,
    ngx_http_variable_value_t *v, uintptr_t data);
static ngx_int_t ngx_http_variable_approx_hit(ngx_http_request_t *r,
    ngx_http_variable_value_t *v, uintptr_t data);
static ngx_int_t ngx_http_variable_resolvable_hit(ngx_http_request_t *r,
    ngx_http_variable_value_t *v, uintptr_t data);
static ngx_int_t ngx_http_variable_resolvable_miss(ngx_http_request_t *r,
    ngx_http_variable_value_t *v, uintptr_t data);
static ngx_int_t ngx_http_variable_upstream_bytes(ngx_http_request_t *r,
    ngx_http_variable_value_t *v, uintptr_t data);
static ngx_int_t ngx_http_variable_gpu_usage(ngx_http_request_t *r,
    ngx_http_variable_value_t *v, uintptr_t data);
static ngx_int_t ngx_http_variable_mem_usage(ngx_http_request_t *r,
    ngx_http_variable_value_t *v, uintptr_t data);
static ngx_int_t ngx_http_variable_resolvable_if_compute(ngx_http_request_t *r,
    ngx_http_variable_value_t *v, uintptr_t data);
...
static ngx_http_variable_t  ngx_http_core_variables[] = {
    ...
    { ngx_string("alternate_uri"), NULL,
      ngx_http_variable_alternate_uri, 0, NGX_HTTP_VAR_NOCACHEABLE, 0 },

    { ngx_string("is_proxy"), NULL,
      ngx_http_variable_is_proxy, 0, NGX_HTTP_VAR_NOCACHEABLE, 0 },
    
    { ngx_string("cache_hit"), NULL,
      ngx_http_variable_cache_hit, 0, NGX_HTTP_VAR_NOCACHEABLE, 0 },

    { ngx_string("approx_hit"), NULL,
      ngx_http_variable_approx_hit, 0, NGX_HTTP_VAR_NOCACHEABLE, 0 },

    { ngx_string("resolvable_hit"), NULL,
      ngx_http_variable_resolvable_hit, 0, NGX_HTTP_VAR_NOCACHEABLE, 0 },

    { ngx_string("resolvable_miss"), NULL,
      ngx_http_variable_resolvable_miss, 0, NGX_HTTP_VAR_NOCACHEABLE, 0 },
    
    { ngx_string("upstream_bytes"), NULL,
      ngx_http_variable_upstream_bytes, 0, NGX_HTTP_VAR_NOCACHEABLE, 0 },

    { ngx_string("gpu_usage"), NULL,
      ngx_http_variable_gpu_usage, 0, NGX_HTTP_VAR_NOCACHEABLE, 0 },
   
    { ngx_string("mem_usage"), NULL,
      ngx_http_variable_mem_usage, 0, NGX_HTTP_VAR_NOCACHEABLE, 0 },

    { ngx_string("resolvable_if_compute"), NULL,
      ngx_http_variable_resolvable_if_compute, 0, NGX_HTTP_VAR_NOCACHEABLE, 0 },
    
    { ngx_string("superresolution_time"), NULL,
      ngx_http_variable_superresolution_time, 0, NGX_HTTP_VAR_NOCACHEABLE, 0 },

    { ngx_string("compute_queue_size_cpu"), NULL,
      ngx_http_variable_compute_queue_size_cpu, 0, NGX_HTTP_VAR_NOCACHEABLE, 0 },

    { ngx_string("compute_queue_size_gpu"), NULL,
      ngx_http_variable_compute_queue_size_gpu, 0, NGX_HTTP_VAR_NOCACHEABLE, 0 },
    
    { ngx_string("is_main_request"), NULL,
      ngx_http_variable_is_main_request, 0, NGX_HTTP_VAR_NOCACHEABLE, 0 },
...
static ngx_int_t
ngx_http_variable_is_main_request(ngx_http_request_t *r,
    ngx_http_variable_value_t *v, uintptr_t data)
{
  ngx_uint_t is_main_request = 0;
  if (r == r->main){
    is_main_request = 1;
  }
  ...
  v->len = ngx_sprintf(p, "%O", is_main_request) - p;
  v->valid = 1;
  v->no_cacheable = 0;
  v->not_found = 0;
  v->data = p;
...
static ngx_int_t
ngx_http_variable_alternate_uri(ngx_http_request_t *r,
    ngx_http_variable_value_t *v, uintptr_t data)
{
  if (r->cache &&
      r->cache->use_alternate > 0){
    ngx_str_t *alt_media = r->headers_in.alternate_objects->elts;
    v->data = alt_media[r->cache->use_alternate - 1].data;
    v->len  = alt_media[r->cache->use_alternate - 1].len;        
  }
  else if(r->cache &&
	  r->cache->use_resolvable > 0){
    ngx_str_t *alt_media = r->headers_in.resolvable_objects->elts;
    v->data = alt_media[r->cache->use_resolvable - 1].data;
    v->len  = alt_media[r->cache->use_resolvable - 1].len;        
  }
  else if(r->upstream &&
	  r->resolvable_from_origin > 0){
    ngx_str_t *alt_media = r->headers_in.resolvable_objects->elts;
    v->data = alt_media[r->resolvable_from_origin - 1].data;
    v->len  = alt_media[r->resolvable_from_origin - 1].len;            
  }
  else{
    v->data = r->uri.data;
    v->len = r->uri.len;
...
static ngx_int_t
ngx_http_variable_is_proxy(ngx_http_request_t *r,
    ngx_http_variable_value_t *v, uintptr_t data)
{
  
  ngx_uint_t is_proxy = 0;
  if (r->cache){
    is_proxy = 1;
  }
  ...
  v->len = ngx_sprintf(p, "%O", is_proxy) - p;
  v->valid = 1;
  v->no_cacheable = 0;
  v->not_found = 0;
  v->data = p;
...
static ngx_int_t
ngx_http_variable_cache_hit(ngx_http_request_t *r,
    ngx_http_variable_value_t *v, uintptr_t data)
{
  
  ngx_uint_t is_cache_hit = 0;
  if (r->upstream &&
  ...
  v->len = ngx_sprintf(p, "%O", is_cache_hit) - p;
  v->valid = 1;
  v->no_cacheable = 0;
  v->not_found = 0;
  v->data = p;
...
static ngx_int_t
ngx_http_variable_approx_hit(ngx_http_request_t *r,
    ngx_http_variable_value_t *v, uintptr_t data)
{

  ngx_uint_t is_approx_hit = 0;
  if (r->upstream &&
      r->upstream->cache_status == NGX_HTTP_CACHE_HIT &&
      r->cache != NULL &&
      r->cache->use_alternate > 0){
  ...
  v->len = ngx_sprintf(p, "%O", is_approx_hit) - p;
  v->valid = 1;
  v->no_cacheable = 0;
  v->not_found = 0;
  v->data = p;
...
static ngx_int_t
ngx_http_variable_resolvable_hit(ngx_http_request_t *r,
    ngx_http_variable_value_t *v, uintptr_t data)
{
  ngx_uint_t is_resolvable_hit = 0;
  if (r->upstream &&
      r->upstream->cache_status == NGX_HTTP_CACHE_HIT &&
      r->cache != NULL &&
      r->cache->use_resolvable > 0){
  ...
  v->len = ngx_sprintf(p, "%O", is_resolvable_hit) - p;
  v->valid = 1;
  v->no_cacheable = 0;
  v->not_found = 0;
  v->data = p;
...
static ngx_int_t
ngx_http_variable_resolvable_miss(ngx_http_request_t *r,
    ngx_http_variable_value_t *v, uintptr_t data)
{
  ngx_uint_t is_resolvable_miss = 0;
  if (r->upstream &&
      r->upstream->cache_status == NGX_HTTP_CACHE_MISS &&
      r->resolvable_from_origin > 0){
    is_resolvable_miss = 1;
  }  
  ...
  v->len = ngx_sprintf(p, "%O", is_resolvable_miss) - p;
  v->valid = 1;
  v->no_cacheable = 0;
  v->not_found = 0;
  v->data = p;
...
static ngx_int_t
ngx_http_variable_upstream_bytes(ngx_http_request_t *r,
    ngx_http_variable_value_t *v, uintptr_t data)
{
  ...
  ngx_uint_t upstream_bytes = 0;
  if (r->upstream &&
      r->upstream->cache_status == NGX_HTTP_CACHE_MISS) {
    upstream_bytes = r->connection->sent;
  }
 
  v->len = ngx_sprintf(p, "%O", upstream_bytes) - p;
  v->valid = 1;
  v->no_cacheable = 0;
  v->not_found = 0;
  v->data = p;
...
static ngx_int_t
ngx_http_variable_superresolution_time(ngx_http_request_t *r,
    ngx_http_variable_value_t *v, uintptr_t data)
{
  u_char  *p;
  ...
  v->len = ngx_sprintf(p, "%O", r->superresolution_time) - p;
  v->valid = 1;
  v->no_cacheable = 0;
  v->not_found = 0;
  v->data = p;
...
static ngx_int_t
ngx_http_variable_compute_queue_size_cpu(ngx_http_request_t *r,
    ngx_http_variable_value_t *v, uintptr_t data)
{
  u_char  *p;
  ...
  ngx_int_t tm = get_compute_cpu_queue_size();

  //ngx_log_debug(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
  //		"Sabby Sabby Sabby: %d", tm);

  
  v->len = ngx_sprintf(p, "%O", tm) - p;
  v->valid = 1;
  v->no_cacheable = 0;
  v->not_found = 0;
  v->data = p;
...
static ngx_int_t
ngx_http_variable_compute_queue_size_gpu(ngx_http_request_t *r,
    ngx_http_variable_value_t *v, uintptr_t data)
{
    u_char  *p;   
    p = ngx_pnalloc(r->pool, NGX_OFF_T_LEN);
    ...
    ngx_int_t tm = get_compute_gpu_queue_size();
    v->len = ngx_sprintf(p, "%O", tm) - p;
    v->valid = 1;
    v->no_cacheable = 0;
    v->not_found = 0;
...
static ngx_int_t
ngx_http_variable_gpu_usage(ngx_http_request_t *r,
    ngx_http_variable_value_t *v, uintptr_t data)
{
  u_char  *p;
  ...
  v->len = ngx_sprintf(p, "%O", r->gpu_usage) - p;
  v->valid = 1;
  v->no_cacheable = 0;
  v->not_found = 0;
  v->data = p;
...
static ngx_int_t
ngx_http_variable_mem_usage(ngx_http_request_t *r,
    ngx_http_variable_value_t *v, uintptr_t data)
{
  ...
  v->len = ngx_sprintf(p, "%O", r->mem_usage) - p;
  v->valid = 1;
  v->no_cacheable = 0;
  v->not_found = 0;
  v->data = p;
...
static ngx_int_t
ngx_http_variable_resolvable_if_compute(ngx_http_request_t *r,
    ngx_http_variable_value_t *v, uintptr_t data)
{

  ngx_uint_t resolvable_if_compute = 0;
  if(r->cache != NULL &&
     r->cache->resolvable_if_compute == 1){
    resolvable_if_compute = 1;
  }
  ...
  v->len = ngx_sprintf(p, "%O", resolvable_if_compute) - p;
  v->valid = 1;
  v->no_cacheable = 0;
  v->not_found = 0;
  v->data = p;
