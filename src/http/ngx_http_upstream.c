...
extern ngx_int_t ngx_http_file_cache_get_quality(ngx_str_t *url, ngx_http_request_t *r);
...
ngx_http_upstream_cache(ngx_http_request_t *r, ngx_http_upstream_t *u)
{
	...
	if (r == r->main &&
        ...
        r->headers_in.resolution.len > 0)
        {
	  ngx_log_debug0(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                       "sabby create resolved cache keys");
          ngx_http_file_cache_create_resolved_keys(r);
        }
        ...
        {
	  ngx_log_debug0(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
			 "sabby create alternate cache keys");

          ngx_http_file_cache_create_alternate_keys(r);
        }
...
ngx_int_t
ngx_http_upstream_update_url(ngx_chain_t *out, ngx_int_t update_quality)
{
  ngx_buf_t *buf = out->buf;

  if (buf->in_file == 0 && buf->pos != buf->last){
    ...
    ngx_http_upstream_t *u, ngx_uint_t do_write)
{
    ...
    if (!r->request_body_no_buffering) {
	  ...
	  //Ani
	  rc = ngx_http_upstream_update_url(out, r->best_quality_to_download);
	  u->cacheable = 0;

	  // Ani: update compute queue size here
        }
        */
	...
	if (out) {
	  ...
	  if (buf->in_file == 0 && buf->pos != buf->last) {
	    //ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
	    //		  "Request body: \"%*s\"", (size_t)(buf->last - buf->pos), buf->pos);
	  }	      
	}
